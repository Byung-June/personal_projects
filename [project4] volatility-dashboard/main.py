...
def fetch_historical_vol_data(currency, period):
    # pull historical price data with a TLS-fingerprinted session
    ticker = yf.Ticker(currency, session=yf_session)
    currency_data = ticker.history(period=period)
    ...
    for days in time_frames:
        currency_data[f'{days}_day_close_vol'] = (
            currency_data['log_return'].rolling(window=days).std() * np.sqrt(365)
        )  # Annualizing the volatility
        currency_data[f'{days}_day_park_vol'] = (
            (1 / (4 * np.log(2)) * (np.log(currency_data['High'] / currency_data['Low']) ** 2))
            .rolling(window=days)
            .mean()
            .pipe(lambda x: np.sqrt(x) * np.sqrt(365))
        )  # Annualizing the Parkinson volatility
        currency_data[f'{days}_day_park_close_ratio'] = (
            currency_data[f'{days}_day_park_vol'] / currency_data[f'{days}_day_close_vol']
        )
